...
### 課題7.5： 全体のレイアウトとグリッドシステム
最後に、全体の配置バランスを整えます。

1. **基本コンテナの適用**
   - 共通レイアウト（`layout.html`）の、メインコンテンツが表示される領域（`<main>` など）を、`container` クラスを持つ `<div>` で囲んでください。また、適宜 `my-4` などの余白を付けて、画面端との間にゆとりを持たせます。
2. **グリッドシステム（配置制御）の利用**
   - **ログイン画面 (`index.html`)**: `row` と `col-md-6 mx-auto` を使い、ログインフォームが画面の中央に適度な幅で表示されるように配置を工夫してください。
   - **メニュー画面 (`book_index.html`)**: 2つの検索フォームが横並び（またはスマートフォン時は縦並び）になるよう、`row` と `col-md-6` を使ってレイアウトしてください。
   - **登録・更新画面**: 入力フォームが横に広がりすぎないよう、グリッドシステムで中央寄りに制限すると見やすくなります。

3. **【最終確認】**
   全ての画面が Bootstrap によって整えられ、ブラウザの幅を変えても崩れないレスポンシブなデザインになっていることを確認して、演習完了となります！
