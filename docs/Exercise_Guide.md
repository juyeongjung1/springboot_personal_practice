...
### 3. 初期テストデータについて
データベースのセットアップ完了時点で、動作確認用の初期データがあらかじめ登録されています。アプリの起動確認やログインテストの際は、以下のデータを使用してください。
...
**■ 初期登録データ（書籍・ジャンル）**
「Java入門」「Spring Boot実践」など9件の書籍データと、「プログラミング」「ビジネス」など5件のジャンルデータが最初から登録されています。詳細な初期データ一覧については、[全体設計書（画面遷移・イメージ図・ER図）](Comprehensive_Design_Specification.md) の巻末を参照してください。
