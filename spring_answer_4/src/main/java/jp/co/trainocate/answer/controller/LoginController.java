...
public class LoginController {
	...
	@PostMapping("/login5")
	public String loginFinal(@Valid UserForm userForm, BindingResult bdResult, HttpSession session, Model model) {
		//入力エラーチェック
		if (bdResult.hasErrors()) {
			return "login_input_final";
		}
		...
		//DB照会後のID&パスワードのチェック
		if (user == null) {
	        model.addAttribute("loginError", "社員番号またはパスワードが誤っています。");
	        return "login_input_final";
	    }
